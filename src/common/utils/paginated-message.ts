...
export interface PaginatedMessageExtraOptions {
  stopInteractionTemplate?: EmbedBuilder | BaseMessageOptions;
  enableMessageCollector?: boolean;
  messageCollectorOptions?: PaginatedMessageCollectorOptions | null;
}

export interface PaginatedMessageCollectorHandlerContext {
  message: Message;
  handler: PaginatedMessage;
  author: User;
  channel: Message['channel'];
  response: Message | AnyInteractableInteraction;
  collector: MessageCollector;
}

interface PaginatedMessageCollectorOptions {
  filter?: CollectorFilter<[Message<boolean>, Collection<string, Message<boolean>>]>;
  run?(context: PaginatedMessageCollectorHandlerContext): Awaitable<unknown>;
}
...
export class PaginatedMessage {
  ...
  /**
   * The collector used for handling messages.
   */
  public messageCollector: MessageCollector | null = null;

  /**
   * Enable the message collector.
   */
  public enableMessageCollector: boolean;

  /**
   * Message collector options.
   */
  public messageCollectorOptions: PaginatedMessageCollectorOptions | null = null;
  ...
  /**
   * The template for this {@link PaginatedMessage} when all interaction collectors are stopped.
   * This will apply to the message when all interaction collectors are stopped
   */
  public stopInteractionTemplate: PaginatedMessageMessageOptionsUnion;
  ...
  public constructor({
    ...
    enableMessageCollector = false,
    messageCollectorOptions = null
  }: PaginatedMessageOptions & PaginatedMessageExtraOptions = {}) {
  ...
  /**
   * Enable/disable the message collector.
   */
  public setEnableMessageCollector(enable: boolean) {
    this.enableMessageCollector = enable;
  }

  /**
   * Set message collector options.
   */
  ...
  public clone(): PaginatedMessage {
    ...
    clone.enableMessageCollector = this.enableMessageCollector;
    clone.messageCollectorOptions = this.messageCollectorOptions;
    return clone;
  }
  ...
  public async getPageOptions(index: number): Promise<PaginatedMessageMessageOptionsUnion | undefined> {
    ...
    return isFunction(page) ? page(index, this.pages, this as any) : page;
  }
   ...
   * @param targetUser The user the handler is for.
   */
  protected setUpMessageCollector(channel: TextBasedChannel, targetUser: User): void {
    if (this.enableMessageCollector) {
      this.messageCollector = new MessageCollector(channel, {
        filter: this.messageCollectorOptions?.filter || ((message) =>
          !isNullish(this.response) && //
  ...
  protected async handlePageLoad(page: PaginatedMessagePage, index: number): Promise<PaginatedMessageMessageOptionsUnion> {
    ...
    const options = isFunction(page) ? await page(index, this.pages, this as any) : page;
  ...
  /**
   * Handles the `collect` event from the message collector.
   * @param targetUser The user the handler is for.
   * @param channel The channel the handler is running at.
   * @param message The message that was received.
   */
  protected async handleMessageCollect(targetUser: User, channel: Message['channel'], message: Message): Promise<void> {
    // Reset the timers
    this.resetAllCollectorTimers();

    if (!this.messageCollectorOptions?.run)
      return;

    await this.messageCollectorOptions.run({
      message: message,
      handler: this as any,
      author: targetUser,
      channel,
  ...
  /**
   * Handles the `end` event from the message collector.
   * @param reason The reason for which the collector was ended.
   */
  protected async handleMessageCollectorEnd(_: Collection<Snowflake, Message<boolean>>, reason: string): Promise<void> {
    // Ensure no race condition can occur where interacting with the message when the paginated message closes would otherwise result in a DiscordAPIError
    if (
      (reason === 'time' || reason === 'idle') &&
      this.response !== null &&
